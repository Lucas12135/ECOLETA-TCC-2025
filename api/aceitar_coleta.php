...
try {
    if ($conn) {
        // Verificar se a coleta existe e se pode ser aceita por este coletor
        ...
        if (!$coleta) {
            ...
            echo json_encode(['success' => false, 'message' => 'Coleta não encontrada ou não pode ser aceita']);
            exit;
        }
        
        // Atualizar a coleta com status agendada e id_coletor
        $stmt_update = $conn->prepare("
            UPDATE coletas 
            SET id_coletor = :id_coletor, status = 'agendada'
            WHERE id = :id_coleta
        ");
        $stmt_update->bindParam(':id_coletor', $id_coletor);
        $stmt_update->bindParam(':id_coleta', $id_coleta);
        
        if ($stmt_update->execute()) {
            echo json_encode(['success' => true, 'message' => 'Coleta aceita com sucesso']);
        } else {
