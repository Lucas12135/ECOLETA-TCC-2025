...
function mostrarResultado(tipo, titulo, mensagem, recarregar = false) {
  ...
  let html = `<div class="modal-resultado-${tipo}">
        <i class="ri-${
          tipo === "sucesso" ? "check-circle" : "error-warning"
        }-fill"></i>
        <div>${mensagem}</div>
    </div>`;
...
document.addEventListener("DOMContentLoaded", function () {
        ...
        if (!e.target.closest("button")) {
          ...
          if (item.classList.contains("expanded")) {
            ...
            detailGroups.forEach((group) => {
              const label =
                group.querySelector(".detail-label")?.textContent?.trim() || "";
              const value =
                group.querySelector(".detail-value")?.textContent?.trim() || "";
  ...
  document.querySelectorAll(".btn-aceitar").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      mostrarConfirmacao(
        "Aceitar Coleta",
        "Você deseja aceitar esta solicitação de coleta?",
        () => {
  ...
  document.querySelectorAll(".btn-concluir").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      document.getElementById("quantidade_coletada").placeholder =
        "Solicitado: " + quantidade + " litros";
  ...
  document
    .getElementById("quantidade_coletada")
  ...
  // Confirmar conclusão
  document
    .getElementById("btnConfirmarConclusao")
    .addEventListener("click", () => {
      const coletaId = document.getElementById("hidden_coleta_id").value;
      const quantidadeColetada = document.getElementById(
        "quantidade_coletada"
      ).value;
      ...
      if (!quantidadeColetada) {
        mostrarResultado(
          "erro",
          "Erro",
          "Por favor, informe a quantidade de óleo coletada"
        );
        return;
      }
      ...
      if (match) {
        ...
        if (quantidadeDigitada > quantidadeMaxima) {
          mostrarResultado(
            "erro",
            "Quantidade Inválida",
            `O máximo permitido é ${quantidadeMaxima.toFixed(
              1
            )} litros (20% acima do esperado de ${quantidadeEsperada} litros).`
          );
          return;
        }
      }
        ...
        .then((data) => {
          if (data.success) {
            ...
            mostrarResultado(
              "erro",
              "Erro",
              "Erro ao concluir coleta: " + (data.message || "Tente novamente")
            );
          }
        })
        .catch((error) => {
  ...
  document.querySelectorAll(".btn-recusar").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      mostrarConfirmacao(
        "Recusar Coleta",
        "Você deseja recusar esta solicitação de coleta?",
        () => {
  ...
  // Fim do processamento de requisições de coleta
});
